...
## Script containing code to test various classifiers performance at quality control of 10X variants
...
rf_CVtest <- classifier.crossValidate(function(x){randomForest(as.factor(cat) ~ Length + Ref + Quality + GC + SINEs + int + coef1 + SDbins + CentBins + TeloBins + LINEs + LTRs,
                                                               data = x,cutoff=c(0.25,0.75),replace=TRUE,ntree=2000)},testData)

svm_CVtest <- classifier.crossValidate(function(x){svm(as.factor(cat) ~ Length + GC + int + coef1 + SDbins + TeloBins + RepTotal,
                                                       data = x,type="C-classification",cost=300,class.weights=setNames(c(1,5),c(1,0)))},testData)

logit_CVtest <- classifier.crossValidate(function(x){train(as.factor(cat) ~ Length + GC + int + coef1 + RepTotal + CentBins + TeloBins + SDs + OtherRepeats,data = x,method='LogitBoost',metric='Kappa',trControl=trainControl('none'))},testData)

knn_CVtest <- classifier.crossValidate(function(x){train(as.factor(cat) ~ Length + GC + RepTotal + CentBins + TeloBins + OtherRepeats + coef1 + int + SDs,data = x,method='knn',metric='Kappa',trControl=trainControl('none'))},testData)

boost_CVtest <- classifier.crossValidate(function(x){train(as.factor(cat) ~ Length + int + Quality + GC + OtherRepeats + coef1 + RepTotal + SINEs + Genotype,
                                                           data = x,method='gbm',metric='Kappa',verbose=FALSE,trControl=trainControl('none'))},testData)

adaboost_CVtest <- classifier.crossValidate(function(x){train(as.factor(cat) ~ Length + int + Quality + GC + OtherRepeats + coef1 + RepTotal + SINEs + Genotype + LINEs + 
                                                                CentBins + TeloBins + LTRs + SimpleRepeats,
                                                              data = x,method='adaboost',metric='Kappa',verbose=FALSE,trControl=trainControl('cv',repeats=5))},testData,divs=5)

ens <- function(y){ensembleclassifier(list(rf=function(x){randomForest(as.factor(cat) ~ Length + Ref + Quality + GC + SINEs + int + coef1 + SDbins + CentBins + TeloBins + LINEs + LTRs,
                                                                       data = x,cutoff=c(0.25,0.75),replace=TRUE,ntree=2000)},
                        ada=function(x){train(as.factor(cat) ~ Length + int + Quality + GC + OtherRepeats + coef1 + RepTotal + SINEs + Genotype + LINEs + 
                                                CentBins + TeloBins + LTRs + SimpleRepeats,
                                              data = x,method='adaboost',metric='Kappa',verbose=FALSE,trControl=trainControl('cv',repeats=5))},
                        logit=function(x){train(as.factor(cat) ~ Length + GC + int + coef1 + RepTotal + CentBins + TeloBins + SDs + OtherRepeats,data = x,method='LogitBoost',metric='Kappa',trControl=trainControl('none'))},
                        boost=function(x){train(as.factor(cat) ~ Length + int + Quality + GC + OtherRepeats + coef1 + RepTotal + SINEs + Genotype,
                                                data = x,method='gbm',metric='Kappa',verbose=FALSE,trControl=trainControl('none'))},
                        knn=function(x){train(as.factor(cat) ~ Length + GC + RepTotal + CentBins + TeloBins + OtherRepeats + coef1 + int + SDs,data = x,method='knn',metric='Kappa',trControl=trainControl('none'))}),
                        y)}
...
save(rf_roc,svm_roc_cost,svm_roc_weight,file = "results/rocCurves.Rdata")
source("scripts/colourBar.R")

## ROC Curves
pdf("figures/rocs.pdf",width = 11,height = 5)

par(fig=c(0,0.33,0,1),mar=c(4,4,4,0),xpd=FALSE,oma=c(1,1,3,1))
rocCurve(rf_roc,pch=20,col=colorRampPalette(c("red","blue"))(19),main="A. Random Forrest")
abline(h=0.85)
colourBar(c(0.33,0.42,0.1,0.9),labels = seq(0.05,0.95,0.1),main = "Weight")

par(fig=c(0.52,0.85,0,1),new=TRUE)
rocCurve(svm_roc_weight,pch=20,col=colorRampPalette(c("red","blue"))(15),main="B. SVM")
abline(h=0.85)
colourBar(c(0.85,0.94,0.1,0.9),labels = seq(1,8,1),main = "Weight")

title("ROC Curves Varying Weighting Parameters",outer = TRUE)
dev.off()
